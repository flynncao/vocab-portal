...
import { scrapeInfoFromCambridgeDictionary, showInfo } from '~/utils/scraper'
...
async function go() {
  if (name.value.trim() !== '') {
    if (scraperMode.value === false) {
      ...
      // const res = await scrapeInfoFromCambridgeDictionary(name.value)
      // console.log('res :>> ', res)
      showInfo()
    }
  }
    ...
    <div class="p-4">
      ...
      <div class="w-full flex flex-col items-center">
        <div class="overflow-hidden">
          <label for="scaperModeSwitcher" class="mr-2"><del>Scaper the Whole Page Instead?</del></label>
          <input id="scaperModeSwitcher" v-model="scraperMode" disabled type="checkbox">
        </div>
